...
namespace Gaia
{
    ...
    public class TerrainSceneActionQueueEntry { public TerrainScene m_terrainScene; public ReferenceChange m_referenceChange; public GameObject referencingObject; public bool m_forced = false; public float m_distance = 0f; public bool m_inFrustum = false; }
    ...
    public class TerrainScene
    {
        ...
        public int m_xCoord = int.MinValue;
        public int m_zCoord = int.MinValue;
        ...
        public void RemoveAllReferences(bool forceSceneRemove = false)
        {
            ...
            UpdateLoadState(m_regularReferences, ref m_regularLoadState, ref m_regularLoadRequested, ref m_regularUnloadRequested, false, true);
            UpdateLoadState(m_impostorReferences, ref m_impostorLoadState, ref m_impostorLoadRequested, ref m_impostorUnloadRequested, true , true);
#endif
        ...
        public void UpdateLoadState(List<GameObject> referenceList, ref LoadState loadState, ref bool loadRequested, ref bool unLoadRequested, bool isImpostor, bool bypassThreshold = false)
        {
            ...
            switch (loadState)
            {
                case LoadState.Loaded:
                    ...
                    {
                        UnloadScene(scene, ref loadRequested, ref unLoadRequested, ref loadState, isImpostor, bypassThreshold);
            ...
            if (Application.isPlaying)
            {
                if (!loadRequested)
                {
                    ...
                    else
                    {
                        ...
                        //The "+1" is to guarantee we get a new time stamp - under race conditions it could happen that no full ms has passed since the last time!
                        tlm.m_lastTerrainLoadedTimeStamp = currentTimeStamp + 1;

                        //Can't continue without a load op - usually this means the scene is not set up in the build settings.
                        if (m_asyncLoadOp == null)
                        {
                            Debug.LogError($"Could not load the scene with the path '{scenePath}'. Please make sure this scene has been included in the Unity Editor Build Settings! There is a button in the Terrain Loader Manager in the 'Terrains' panel from which you can add all terrain scenes to the Build Settings with one click.");
                            return;
                        }
        ...
        private void UnloadScene(Scene scene, ref bool loadRequested, ref bool unLoadRequested, ref LoadState loadState, bool isImpostor, bool bypassTreshold = false)
        {
            ...
            if (Application.isPlaying)
            {
                ...
                //The "+1" is to guarantee we get a new time stamp - under race conditions it could happen that full ms has passed since the last time!
                tlm.m_lastTerrainLoadedTimeStamp = currentTimeStamp + 1;
            }
            ...
            if (scene.path == m_scenePath || scene.path == m_colliderScenePath)
            {
                if (m_regularLoadState != LoadState.Loaded)
                {
                    ...
                    foreach (GameObject go in scene.GetRootGameObjects())
                    {
                        ...
                        {
                            m_terrainObj = go;
                            go.transform.position = m_bounds.center - TerrainLoaderManager.Instance.GetOrigin() - new Vector3Double(m_bounds.size.x / 2f, 0f, m_bounds.size.z / 2f);
                        }
                        go.SetActive(true);
                    }
        ...
        private void SceneLoadCompletedHandling()
        {
            ...
            foreach (GameObject go in scene.GetRootGameObjects())
            {
                ...
                {
                    go.transform.position += (Vector3)TerrainLoaderManager.Instance.GetOrigin();
