...
namespace Gaia
{
    ...
    public class TerrainLoaderManager : MonoBehaviour
    {
        ...
        public void UpdateTerrainLoadState(BoundsDouble loadingBoundsRegular = null, BoundsDouble loadingBoundsImpostor = null, GameObject requestingObject = null, float minDistance = 0, float maxDistance = 0, float minThresholdMS = 0, float maxThresholdMS = 0, Camera frustumCamera = null)
        {
            ...
            //no bounds? nothing to do, we can just create an empty list
            if (loadingBoundsRegular == null)
            {
                regularTerrainScenesToLoad = new List<TerrainScene>();
            }
            if (loadingBoundsImpostor == null)
            {
            ...
            foreach (TerrainScene terrainScene in m_terrainSceneStorage.m_terrainScenes.Where(x => x.HasImpostorReference(requestingObject) && !impostorTerrainScenesToLoad.Contains(x)))
            {
                float distance;
            ...
            foreach (TerrainScene terrainScene in m_terrainSceneStorage.m_terrainScenes.Where(x => x.HasRegularReference(requestingObject) && !regularTerrainScenesToLoad.Contains(x)))
            {
                float distance;
                if (frustumCamera == null)
                {
